    ...
    const query = connect.con.query('SELECT * FROM utilizador_por_equipa WHERE id_user = ? AND id_equipa = ? AND lider = 1', scan,
        function (err, rows, fields) {
            ...
            if (!rows.length) {
                ...
                const secondQuery = connect.con.query('SELECT * FROM utilizador_por_equipa WHERE id_equipa = ?', id_equipa,
                function (err, rows, fields) {
                    console.log(secondQuery.sql);
                    if (rows.length == 1) {
                        return next();
                    } else {
                        return res.status(jsonMessagesUser.user.unauthorizedOwnerLeaving.status).send(jsonMessagesUser.user.unauthorizedOwnerLeaving);
                    }
                })
            }
        });
